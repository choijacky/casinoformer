...
create_projection = partial(gaussian_orthogonal_random_matrix, nb_rows = R, nb_columns = D, scaling = 0)
projection_matrix = create_projection()

seq = torch.exp(torch.inner(q, k)/math.sqrt(D))

dist = torch.nn.functional.normalize(seq, p=1.0, dim=-1)
print(dist)

create_kernel = partial(softmax_kernel, projection_matrix = projection_matrix, device=None)
q = create_kernel(q, is_query = True)
k = create_kernel(k, is_query = False)

seq_approx = torch.inner(q, k)
dist_approx = torch.nn.functional.normalize(seq_approx, p=1.0, dim=-1)
print(dist_approx)